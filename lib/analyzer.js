...
const THEME_INSTRUCTIONS = {
  overall: '',
  love: '【テーマ指定: 恋愛・パートナーシップ重点】感情線・結婚線・金星帯・金星丘・月丘の解釈を特に詳細に展開すること。恋愛傾向・パートナーシップ・理想の関係性・出会いのタイミングを中心に、他のセクションより3倍以上の分量で分析すること。',
  career: '【テーマ指定: 仕事・キャリア重点】運命線・太陽線・知能線・水星線・木星丘の解釈を特に詳細に展開すること。向いている職種・働き方・キャリアの転換期・2026〜2027年の仕事戦略を中心に、他のセクションより3倍以上の分量で分析すること。',
  health: '【テーマ指定: 健康・体力重点】生命線・水星線・金星丘・月丘の解釈を特に詳細に展開すること。体質・エネルギータイプ・注意すべき健康傾向・セルフケア方法を中心に、他のセクションより3倍以上の分量で分析すること。',
};

const THEME_FOCUS_SCHEMA = {
  overall: '',
  love: `  "theme_focus": {
    "label": "恋愛・パートナーシップ 深堀り分析",
    "attraction_type": "手相から読み取れる、惹かれやすい相手のタイプ（2〜3文）",
    "relationship_pattern": "繰り返しやすい恋愛パターンと、その背景にある心理（2文）",
    "ideal_partner": "この手相が示す最も相性の良いパートナー像（具体的に、2文）",
    "timing": "恋愛の転機・出会いが訪れやすい時期の傾向（断定せず可能性として、2文）",
    "action": "今すぐできる恋愛運向上のための具体的な行動（2文）"
  },`,
  career: `  "theme_focus": {
    "label": "仕事・キャリア 深堀り分析",
    "best_field": "手相が示す最も力を発揮できる職種・分野（具体的な職業名も含めて、2〜3文）",
    "work_style": "向いている働き方（独立・組織・フリーランス・副業等、理由込みで2文）",
    "timing": "キャリアの転換期・大きなチャンスが来やすい時期（断定せず、2文）",
    "strength": "仕事における最大の武器・他者との差別化ポイント（2文）",
    "strategy_2026": "2026〜2027年のキャリア戦略（具体的な行動提案を含む、2〜3文）"
  },`,
  health: `  "theme_focus": {
    "label": "健康・体力 深堀り分析",
    "constitution": "手相から読み取れる体質・エネルギータイプ（2文）",
    "caution": "注意すべき健康上の傾向（断定せず傾向として、対策も含む、2文）",
    "energy_rhythm": "エネルギーが高まりやすい時期・活動リズムの傾向（2文）",
    "self_care": "この手相のタイプに合ったセルフケア・健康習慣（具体的に、2文）",
    "forecast_2026": "2026年の健康・体力の傾向と注意点（2文）"
  },`,
};

function buildInterpretationPrompt(observations, userSelectedHand, theme = 'overall', customQuestion = '', userAge = '', userGender = '') {
  ...
  const profileParts = [];
  if (userAge) profileParts.push(`年齢: ${userAge}歳`);
  if (userGender && userGender !== 'unspecified') profileParts.push(`性別: ${userGender}`);
  const profileNote = profileParts.length
    ? `【ユーザープロフィール】${profileParts.join('・')}\n→ 年齢・性別に合わせた現実的な時期感・ライフステージを考慮した鑑定を行うこと。\n`
    : '';
...
${profileNote}${themeInstruction ? themeInstruction + '\n' : ''}
【文体の厳守事項 — 全文に適用すること】
- 全ての文を「です・ます調」で統一する
- 文末は「〜傾向があります」「〜可能性があります」「〜と読み取れます」「〜を示しています」で統一する
- 「〜でしょう」「〜かもしれません」「〜ですね」は使わない
- 体言止め（例:「強い意志。」）は使わない。必ず述語で締める
- 感嘆符（！）は使わない
- 主語は「あなたは〜」で明確にする
- 同じ表現・フレーズを同一セクション内で繰り返さない
...
8. 2026〜2027年の運勢は「可能性」「傾向」として記述し、断定しない
...

{
  ...
  "lines": {
    "life": {
      ...
      "modern_lens": "2026〜2027年の視点からのコメント（1文）"
    },
    "heart": {
      "title": "感情線",
      "emoji": "❤️",
    ...
    "fate": {
      ...
      "modern_lens": "2026〜2027年の働き方への言及"
    },
    "sun": {
      "title": "太陽線",
      "emoji": "☀️",
  ...
  "annual_fortune": {
    "2026": "2026年の運勢の傾向と可能性（1〜2文。断定しない）",
    "2027": "2027年への展望（1〜2文。断定しない）",
    "focus_area": "特に意識すると良い分野（例：仕事・恋愛・健康）"
  },
  "life_advice": "手相全体から導き出される人生アドバイス（2〜3文。温かく、具体的に。です・ます調で）",${THEME_FOCUS_SCHEMA[theme] ? '\n' + THEME_FOCUS_SCHEMA[theme] : ''}${customQuestion ? `
  "custom_question": {
    "question": "${customQuestion.replace(/"/g, '\\"')}",
    "answer": "手相から読み取れるこの質問への答え（2〜3文。断定せず可能性・傾向として記述）",
    "timing_hint": "時期・タイミングに関する示唆（質問が時期を問う場合。なければ空文字）"
  },` : ''}
  "lucky": {
...
// =========================================================
// 画質が高い場合はStep 3をスキップしてコストを削減
// =========================================================
function shouldSkipVerification(observations) {
  const q = observations.image_quality;
  if (q.overall !== 'excellent' && q.overall !== 'good') return false;
  const lines = observations.lines || {};
  const highCount = ['life_line', 'heart_line', 'head_line']
    .filter(k => lines[k]?.confidence === 'high' && lines[k]?.visible === true).length;
  return observations.overall_readability === 'high' && highCount >= 2;
}

// =========================================================
// メイン分析関数
// =========================================================
export async function analyzePalm(base64, mediaType, userSelectedHand = 'unclear', theme = 'overall', customQuestion = '', userAge = '', userGender = '') {
  ...
  try {
    const step2 = await withTimeout(
      client.messages.create({
        ...
        messages: [{
          role: 'user',
          content: buildInterpretationPrompt(observations, userSelectedHand, theme, customQuestion, userAge, userGender),
        }],
      }),
      90_000
    );
  ...
  // --- Step 3: 自己検証（高品質画像はスキップしてコスト削減）---
  let verifiedInterpretation = interpretation;
  if (shouldSkipVerification(observations)) {
    // 画質excellent・信頼度highが揃っている場合は検証不要
  } else try {
...
// 両手比較: 合成プロンプト
// =========================================================
function buildComparisonPrompt(rightInterp, leftInterp) {
  return `右手（現在・努力の手）の鑑定:
${JSON.stringify(rightInterp, null, 2)}

左手（潜在能力・先天の手）の鑑定:
${JSON.stringify(leftInterp, null, 2)}

=== 両手比較分析 ===
上記の右手・左手の鑑定を総合し、以下のJSON形式で両手比較鑑定を日本語で生成してください。

【観点】
- 右手（現在）: 今の状態・努力・これからの方向性
- 左手（潜在）: 生まれ持った才能・まだ活かされていないポテンシャル
- 一致 → 運命が現実に反映されている部分（既に開花している才能）
- ズレ → 潜在能力が未活用の部分（成長・開花の余地）

以下のJSON形式のみで回答してください:

{
  "right_summary": "右手（現在の手）のポイントまとめ（2〜3文）",
  "left_summary": "左手（潜在能力の手）のポイントまとめ（2〜3文）",
  "consistency": "両手で一致している特質（既に開花している部分）（1〜2文）",
  "gap": "両手でズレがある部分（潜在能力が未活用の領域）（1〜2文）",
  "growth_opportunity": "比較から見える最大の成長・開花チャンス（2文）",
  "overall_insight": "両手を統合した総合的な人生の洞察（3〜4文。温かく深い内容で）",
  "fortune_2026": "2026年の展望（両手を統合した読み・2文。断定しない）",
  "fortune_2027": "2027年への展望（2文。断定しない）",
  "life_advice": "両手比較から導き出す最も重要な人生アドバイス（2〜3文）",
  "lucky": {
    "color": "両手の特性を統合したラッキーカラー",
    "number": "ラッキーナンバー（数字のみ）",
    "keyword": "2026年のキーワード（一言）"
  },
  "closing": "両手比較鑑定の締めくくり（1〜2文。希望と励ましを込めて）"
}`;
}

// =========================================================
// 両手比較分析: メイン関数
// 右手・左手を並列でStage1/2まで実行し、合成Stageで統合
// =========================================================
export async function analyzeComparison(rightBase64, rightType, leftBase64, leftType, theme = 'overall', customQuestion = '') {
  // Stage 1: 両手を並列で観察
  let rightObs, leftObs;
  try {
    [rightObs, leftObs] = await Promise.all([
      withTimeout(client.messages.create({
        model: 'claude-haiku-4-5-20251001',
        max_tokens: 2000,
        messages: [{ role: 'user', content: [
          { type: 'image', source: { type: 'base64', media_type: rightType, data: rightBase64 } },
          { type: 'text', text: OBSERVATION_PROMPT },
        ]}],
      })).then(r => parseJSON(r.content[0].text)),
      withTimeout(client.messages.create({
        model: 'claude-haiku-4-5-20251001',
        max_tokens: 2000,
        messages: [{ role: 'user', content: [
          { type: 'image', source: { type: 'base64', media_type: leftType, data: leftBase64 } },
          { type: 'text', text: OBSERVATION_PROMPT },
        ]}],
      })).then(r => parseJSON(r.content[0].text)),
    ]);
  } catch (err) {
    if (err.message === 'API_TIMEOUT') throw new Error('分析がタイムアウトしました。再試行してください。');
    throw new Error(`観察フェーズエラー: ${err.message}`);
  }

  // 品質チェック
  const rq = rightObs.image_quality;
  const lq = leftObs.image_quality;
  if (!rq.can_analyze || rq.palm_visibility === 'not_palm') {
    return { error: true, message: '右手の画像: ' + buildQualityErrorMessage(rq) };
  }
  if (!lq.can_analyze || lq.palm_visibility === 'not_palm') {
    return { error: true, message: '左手の画像: ' + buildQualityErrorMessage(lq) };
  }

  // Stage 2: 両手を並列で解釈（キャッシュ共有のため同じDBコンテキストを使用）
  let rightInterp, leftInterp;
  const dbContext = buildDatabaseContext();
  try {
    [rightInterp, leftInterp] = await Promise.all([
      withTimeout(client.messages.create({
        model: 'claude-sonnet-4-6',
        max_tokens: 4000,
        system: [{ type: 'text', text: dbContext, cache_control: { type: 'ephemeral' } }],
        messages: [{ role: 'user', content: buildInterpretationPrompt(rightObs, 'right', theme, customQuestion) }],
      }), 90_000).then(r => parseJSON(r.content[0].text)),
      withTimeout(client.messages.create({
        model: 'claude-sonnet-4-6',
        max_tokens: 4000,
        system: [{ type: 'text', text: dbContext, cache_control: { type: 'ephemeral' } }],
        messages: [{ role: 'user', content: buildInterpretationPrompt(leftObs, 'left', theme, customQuestion) }],
      }), 90_000).then(r => parseJSON(r.content[0].text)),
    ]);
  } catch (err) {
    if (err.message === 'API_TIMEOUT') throw new Error('解釈生成がタイムアウトしました。再試行してください。');
  ...
  try {
    const synth = await withTimeout(
      client.messages.create({
        model: 'claude-sonnet-4-6',
        max_tokens: 3000,
        messages: [{ role: 'user', content: buildComparisonPrompt(rightInterp, leftInterp) }],
      }),
      90_000
    );
    comparison = parseJSON(synth.content[0].text);
  } catch (err) {
    throw new Error(`統合フェーズエラー: ${err.message}`);
  }
  ...
  return {
    ...
    mode: 'comparison',
    comparison,
    rightInterpretation: rightInterp,
    leftInterpretation: leftInterp,
    quality: {
      right: { overall: rq.overall, readability: rightObs.overall_readability },
      left: { overall: lq.overall, readability: leftObs.overall_readability },
    },
  };
}
