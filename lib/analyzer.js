...
function buildInterpretationPrompt(observations, userSelectedHand, theme = 'overall') {
...
// メイン分析関数
// =========================================================
export async function analyzePalm(base64, mediaType, userSelectedHand = 'unclear', theme = 'overall') {
  ...
  try {
    const step2 = await withTimeout(
      client.messages.create({
        ...
        messages: [{
          role: 'user',
          content: buildInterpretationPrompt(observations, userSelectedHand, theme),
        }],
      }),
      90_000
    );
...
// =========================================================
export async function analyzeComparison(rightBase64, rightType, leftBase64, leftType, theme = 'overall') {
  // Stage 1: 両手を並列で観察
  ...
  try {
    [rightInterp, leftInterp] = await Promise.all([
      withTimeout(client.messages.create({
        ...
        messages: [{ role: 'user', content: buildInterpretationPrompt(rightObs, 'right', theme) }],
      }), 90_000).then(r => parseJSON(r.content[0].text)),
      withTimeout(client.messages.create({
        model: 'claude-sonnet-4-6',
        max_tokens: 4000,
        system: [{ type: 'text', text: dbContext, cache_control: { type: 'ephemeral' } }],
        messages: [{ role: 'user', content: buildInterpretationPrompt(leftObs, 'left', theme) }],
      }), 90_000).then(r => parseJSON(r.content[0].text)),
    ]);
  } catch (err) {
    if (err.message === 'API_TIMEOUT') throw new Error('解釈生成がタイムアウトしました。再試行してください。');
