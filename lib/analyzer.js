...
const THEME_INSTRUCTIONS = {
  overall: '',
  love: '【テーマ指定: 恋愛・パートナーシップ重点】感情線・結婚線・金星帯・金星丘の解釈を特に詳細に。恋愛傾向・パートナーシップのあり方・理想の関係性を中心に鑑定すること。',
  career: '【テーマ指定: 仕事・キャリア重点】運命線・太陽線・知能線・水星線の解釈を特に詳細に。キャリアの方向性・向いている職種・2026〜2027年の仕事運を中心に鑑定すること。',
  health: '【テーマ指定: 健康・体力重点】生命線・水星線・金星丘・月丘の解釈を特に詳細に。体質・健康上の注意点・エネルギー管理のコツを中心に鑑定すること。',
};

function buildInterpretationPrompt(observations, userSelectedHand, theme = 'overall', customQuestion = '') {
...

{
  ...
  "life_advice": "手相全体から導き出される人生アドバイス（2〜3文。温かく、具体的に）",${customQuestion ? `
  "custom_question": {
...
// メイン分析関数
// =========================================================
export async function analyzePalm(base64, mediaType, userSelectedHand = 'unclear', theme = 'overall', customQuestion = '') {
  ...
  try {
    const step2 = await withTimeout(
      client.messages.create({
        ...
        messages: [{
          role: 'user',
          content: buildInterpretationPrompt(observations, userSelectedHand, theme, customQuestion),
        }],
      }),
      90_000
    );
